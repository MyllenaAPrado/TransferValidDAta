...
from model.mink_layers import (
    assign_feats,
    SparseTensorLayerNorm,
    SparseTensorLinear,
)
from model.sparse_dl.attn.attn_coff import (
    SelfAttnAIOFunction,
    PosEmb,
    TableDims,
    IndexMode,
...
import model.sparse_dl.knn
from model.sparse_dl.knn import KNN
...
class SwinTransformer(nn.Module):
    def __init__(self, patches_resolution, channels=[3,3,3,3], quant_size=4, 
                 down_stride =3, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
        ...
        self.layers = nn.ModuleList([
            BasicLayer(
                ...
                out_channels=channels[i+1] if i < self.num_layers-1 else None,
                #cRSE=cRSE,
                #fp16_mode=fp16_mode
                ) 
